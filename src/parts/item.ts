...
export class Item extends MyObject3D {
  ...
  public setSize(w:number, h:number):void {
    ...
    if(Util.hit(2)) color = Param.instance.baseColorB
  ...
  protected _update():void {
    ...
    // this._conBlock.scale.y = this._conBlock.scale.x * 0.5
    this._conBlock.scale.z = this._conBlock.scale.x

    this._conLine.scale.y = this._conLine.scale.x * 0.2
    this._conLine.scale.z = this._conLine.scale.x * 0.2

    this.rotation.z = Util.radian(Util.mix(0, 360 * this._noise.y, this._showRate.val))
    // this.position.x = this.itemId * 1
  }
